...
export function IntegrationsPage() {
	...
	const patchMcp = (patch: Partial<McpDraft>) =>
		setMcpDraft((d) => ({ ...d, ...patch }));
	...
	const submitRegister = async () => {
		...
		try {
			...
			const payload: { base_url: string; name?: string } = {
				base_url: regDraft.base_url.trim(),
			};
	...
	return (
		<div ref={rootRef} className="page-container">
										...
										{servers.map((s) => (
											...
											>
												...
												<td className="px-4 py-2.5">
													<Badge tone={TRANSPORT_TONE[s.transport] ?? "gray"}>
														{s.transport}
													</Badge>
												</td>
												...
												<td className="px-4 py-2.5 font-mono text-xs text-fog">
													...
													{Array.isArray(s.tools_cache)
														? s.tools_cache.length
														: 0}
												</td>
				...
				<div className="space-y-3">
					...
					{debugOutput && (
						<div>
							...
							<pre className="code-block max-h-64 whitespace-pre-wrap">
								{debugOutput}
							</pre>
						</div>
					)}
				...
				<div className="space-y-3">
					<div>
						...
						<Input
							...
							onChange={(e) =>
								setRegDraft((d) => ({ ...d, base_url: e.target.value }))
							}
						/>
					</div>
					<div>
						...
						<Input
							...
							onChange={(e) =>
								setRegDraft((d) => ({ ...d, name: e.target.value }))
							}
						/>
					</div>
				</div>
			...
			<ConfirmDialog
				...
				content={
					delTarget ? `确认删除「${delTarget.label}」？该操作不可恢复。` : ""
				}
				danger
				confirmText="删除"
				loading={deleting}
