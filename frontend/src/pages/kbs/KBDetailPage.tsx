		...
		<div ref={pageRef} className="page-container">
			...
			{kb && (
				<div className="mb-6 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-ash">
					<span>
						chunk_size{" "}
						<span className="font-mono text-mist">{kb.chunk_size}</span>
					</span>
					<span>
						chunk_overlap{" "}
						<span className="font-mono text-mist">{kb.chunk_overlap}</span>
					</span>
								...
								{docs.map((doc) => (
									...
									>
										<td className="max-w-xs px-2 py-2.5">
											<p
												className="truncate font-medium text-bone"
												title={doc.filename}
											>
												{doc.filename}
											</p>
											...
											{doc.status === "failed" && doc.error && (
												<p
													className="mt-0.5 truncate text-[11px] text-coral"
													title={doc.error}
												>
													{doc.error}
												</p>
										...
										<td className="px-2 py-2.5 text-xs text-fog">
			...
			<section className="panel mt-6 p-5">
				...
				<div className="mt-4 space-y-2">
					{refs === null ? null : refs.length === 0 ? (
						...
						refs.map((ref, i) => (
							<div
								key={ref.chunk_id} //{`${ref.chunk_id}-${i}`}
							...
							>
								<div className="mb-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
									...
									{ref.page > 0 && (
										<span className="text-ash">第 {ref.page} 页</span>
									)}
									{/* 相似度得分：Teal 功能色 */}
