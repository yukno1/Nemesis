...
import type { Message, RAGRef, TraceStep } from "@/types/chat";
import type { StreamingMsg } from "@/stores/chat";
import { Collapse, streamingToSteps, TraceTimeline } from "./TraceTimeline";

/* ---- 引用列表（refs 事件 / 消息 refs 字段） ---- */

function RefsList({ refs }: { refs: RAGRef[] }) {
		...
		<Collapse title={`引用来源`} badge={`${refs.length} 条`}>
			<ul className="space-y-2">
				{refs.map((r, i) => (
					<li
						key={r.chunk_id}
						className="rounded-md border border-smoke/50 bg-void/60 p-2 text-xs"
					>
						<div className="mb-1 flex items-center gap-2 text-fog">
							...
							<span className="ml-auto tabular-nums text-ash">
								{r.score.toFixed(3)}
							</span>
						</div>
		...
		>
			<div
				className={clsx(
					...
					isUser ? "border-lime/30 bg-lime/5" : "border-smoke/60 bg-obsidian",
				)}
			>
				{/* 多智能体命中专家标记 */}
				{!isUser && expert && (
					<div className="mb-2 text-[10px] text-violet">
						⇢ 由「{expert}」处理
					</div>
				)}
