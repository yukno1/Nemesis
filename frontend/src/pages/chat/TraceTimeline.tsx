...
function StepRow({ step }: { step: TraceStep }) {
	...
	return (
		<li className={clsx("relative pl-6", failed && "text-coral")}>
			...
			>
				<span
					className={clsx("font-medium", failed ? "text-coral" : "text-mist")}
				>
			...
			>
				{step.content && (
					<p className="mt-1 whitespace-pre-wrap text-xs leading-relaxed text-fog">
						{step.content}
					</p>
				)}
				{step.args && Object.keys(step.args).length > 0 && (
					<pre className="code-block mt-1 text-[11px]">
						{JSON.stringify(step.args, null, 2)}
					</pre>
				)}
				{step.result && (
					<pre className="code-block mt-1 max-h-40 text-[11px]">
